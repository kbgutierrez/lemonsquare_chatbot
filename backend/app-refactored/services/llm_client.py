...
def create_main_llm(temperature: float | None = None) -> ChatGroq:
    ...
    from app.services.settings_service import get_active_settings
    from sqlalchemy.orm import Session
    from app.core.database import SessionChatbot
    db = SessionChatbot()
    try:
        config = get_active_settings(db)
        temp = temperature if temperature is not None else (float(config.Temperature) if config.Temperature else 0.2)
        model = config.ActiveModel or "llama-3.3-70b-versatile"
